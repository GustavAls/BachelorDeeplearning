...
class AutoAugment(object):
    def __init__(self, randrange):
        self.policies = [
            ['Invert', 0.1, 7, 'Contrast', 0.2, 6],
            ['Rotate', 0.7, 2, 'TranslateX', 0.3, 9],
            ['Sharpness', 0.8, 1, 'Sharpness', 0.9, 3],
            ['ShearY', 0.5, 8, 'TranslateY', 0.7, 9],
            ['AutoContrast', 1, 8, 'Equalize', 1, 2],
            ['ShearY', 1, 7, 'Posterize', 1, 7],
            ['Color', 1, 3, 'Brightness', 1, 7],
    ...
    def __call__(self, img, do_print):
        img = apply_policy(img, self.policies[self.randrange])
        if do_print:
            print("Policy: " + str(self.policies[self.randrange]))
        return img
...
def apply_policy(img, policy):
    ...
    #Has to be deterministic to make a meaningful plot
    img = operations[policy[3]](img, policy[5])

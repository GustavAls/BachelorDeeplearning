...
def main():
    ...
    for cv in cv_set:
        ...
        dataset_train_eval = utils.ISICDataset(mdlParams, 'trainIndEval')
        # For val
        ...
        modelVars['dataloader_trainIndEval'] = DataLoader(dataset_train_eval, batch_size=mdlParams['multiCropEval'], shuffle=True,
                                                      num_workers=num_workers, pin_memory=True, drop_last=True)
        ...
        for step in range(start_epoch, mdlParams['training_steps'] + 1):
            ...
            if step % mdlParams['display_step'] == 0 or step == 1 or step == 101:
                ...
                if mdlParams['classification']:
                    ...
                    cv, step, mdlParams['training_steps'], int(duration / 3600), int(np.mod(duration, 3600) / 60),
                    int(np.mod(np.mod(duration, 3600), 60))) + time.strftime("%d.%m.-%H:%M:%S", time.localtime()))
                    print("Loss on ", eval_set, "set: ", loss, " Accuracy: ", accuracy, " F1: ", f1, " (best WACC: ",
